export const SIGNUP_PROJECT_REQUEST= "SIGNUP_PROJECT_REQUEST" 
export const SIGNUP_PROJECT_SUCCESS= "SIGNUP_PROJECT_SUCCESS" 
export const SIGNUP_PROJECT_FAIL= "SIGNUP_PROJECT_FAIL" 

export const LOGIN_PROJECT_REQUEST= "LOGIN_PROJECT_REQUEST" 
export const LOGIN_PROJECT_SUCCESS= "LOGIN_PROJECT_SUCCESS" 
export const LOGIN_PROJECT_FAIL= "LOGIN_PROJECT_FAIL"

export const LOGOUT_PROJECT= "LOGOUT_PROJECT"

export const GET_USERDETAIL_REQUEST='GET_USERDETAIL_REQUEST'
export const GET_USERDETAIL_SUCCESS='GET_USERDETAIL_SUCCESS'
export const GET_USERDETAIL_FAIL='GET_USERDETAIL_FAIL'

export const DETECT_EMOTION_REQUEST = 'DETECT_EMOTION_REQUEST';
export const DETECT_EMOTION_SUCCESS = 'DETECT_EMOTION_SUCCESS';
export const DETECT_EMOTION_FAIL = 'DETECT_EMOTION_FAIL';


export const GET_LECTURES_REQUEST = 'GET_LECTURES_REQUEST';
export const GET_LECTURES_SUCCESS = 'GET_LECTURES_SUCCESS';
export const GET_LECTURES_FAIL = 'GET_LECTURES_FAIL';

export const MENTORS_REQUEST = 'MENTORS_REQUEST';
export const MENTORS_SUCCESS = 'MENTORS_SUCCESS';
export const MENTORS_FAIL = 'MENTORS_FAIL';

export const POST_BLOG_REQUEST = 'POST_BLOG_REQUEST';
export const POST_BLOG_SUCCESS = 'POST_BLOG'
export const POST_BLOG_FAIL = 'POST_BLOG_FAIL';

export const LECTURE_RECOMMEND_REQUEST = 'LECTURE_RECOMMEND_REQUEST';
export const LECTURE_RECOMMEND_SUCCESS = 'LECTURE_RECOMMEND_SUCCESS';
export const LECTURE_RECOMMEND_FAIL = 'LECTURE_RECOMMEND_FAIL';


export const STUDENT_CREATE_REQUEST = 'STUDENT_CREATE_REQUEST';
export const STUDENT_CREATE_SUCCESS = 'STUDENT_CREATE_SUCCESS';
export const STUDENT_CREATE_FAIL = 'STUDENT_CREATE_FAIL';


export const STUDENT_PROFILE_CREATE_REQUEST = 'STUDENT_PROFILE_CREATE_REQUEST';
export const STUDENT_PROFILE_CREATE_SUCCESS = 'STUDENT_PROFILE_CREATE_SUCCESS';
export const STUDENT_PROFILE_CREATE_FAIL = 'STUDENT_PROFILE_CREATE_FAIL';

